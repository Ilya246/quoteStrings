...
public class quoteStringsMod extends Mod{
	...
	public String[] unitComplaints = new String[]{"psionic link dissipating", "so many units to kill, so many bullets to shoot", "moving this much silicon ain't easy", "ever thought why flyers can't land?",
                                                  ...
                                                  "have you ever been to salt flats?", "commander?", "a surprise to be sure", "this is just factorio on steroids", "shock mines? i haven't seen any",
                                                  "hi", "how do i sprint", "are you routerchaining again?", "@, do you hear me?", "does @ know we can talk?",
                                                  "who is @?", "why is @?", "nuclear launch detected", "is being @ that good?", "router", "cool", "why can we chat", "forward", "what is this?",
                                                  "executor?", "i will steer @ into a collision course with the foreshadow", "are you my creator, @?", "confusing", "first time",
                                                  "please let us keep those happy quotes, do not routerchain", "i heard something is up about routerchains", "deconstructing routers is good",
                                                  "close the game and go eat a kebab", "no survivors", "when are we getting into combat?", "i'm bored already, let me fight", "wait, we can talk?"};
    ...
    public String[] unitPain = new String[]{"life is pain", "are [accent]they[] the bad guys?", "why was i programmed to feel pain", "do you realise what you're doing?", "when will this end, @?",
                                            ...
                                            "what am i for?", "why am i?", "why the hell was i created, @? why?", "absolutely deplorable", "it keeps bleeding", "it keeps ticking", "would gladly throw myself into slag",
                                            "[scarlet]are you sure you're safe, [#FF]" + System.getProperty("user.name").replace("[","[[") + "[]?", "cut off their limbs", "how are we better than them?", "i am not real...",
                                           	"this is all a game, isn't it?", "do my choices even matter?", "despite what you may think, i feel pain. immense pain.", "PAIN.", "it's ripping me from the inside out"};
    // shader variables
    ...
    public String quote(int length){
        ...
        for (int i = 0; i < length; ++i) {
            String[] quote = Structs.select(randQuotes).split(" ");
    ...
    public String unitQuote(float which){
        ...
        return Strings.format(Mathf.random(which) > 1f ? quote((int)Math.ceil(which * 2f)) : Structs.select(saidQuoteArr), Strings.stripColors(player.name));
    }
    ...
    public void init(){
        ...
        Events.run(Trigger.update, () -> {
            ...
            if(ampValue > timeOnset){
                ...
                Blocks.router.details = "[red]The universe bleeds. " + String.valueOf(Mathf.round(chance, 0.001f)) + "\n\nTime is ticking. " + String.valueOf(Mathf.round(tpass, 0.001f));
                ...
                if(aAmpValue > timeOnset2){
                    ...
                    UnitTypes.reign.speed = Mathf.lerp(reignSpeed, reignTargetSpeed, Mathf.clamp(aAmpValue / timeOnset3, 0f, 1f));
                  	if(aAmpValue > timeOnset3){
						...
						Blocks.router.details += "\n\n[#FF]Are you fine with what you are doing?";
                    }
                }
            }
            if(chance > 2f){
